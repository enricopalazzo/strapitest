...
module.exports = {
  ...
  add: async (values) => {
    ...
    const entry = await Campaign.create(data);

    // Create relational data and return the entry.
    return Campaign.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Campaign.associations.map(a => a.alias));
    const data = _.omit(values, Campaign.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Campaign.update(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Campaign.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Campaign.attributes).reduce((acc, curr) => {
      switch (Campaign.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
